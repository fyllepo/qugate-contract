...
**Author**: phileepphilop (Discord) — full-stack contractor (TypeScript, Python)  
...
**Contract Index**: 24 (TBD)  
**Author**: fyllepo  
**Repository**: [github.com/fyllepo/qugate-contract](https://github.com/fyllepo/qugate-contract)  
**Status**: Tested on local testnet (core-lite). All 5 modes verified, identified abuse/attack vectors tested, stress tested with 50 concurrent gates, multi-sender convergence verified, gate lifecycle tested. Unit tests added.
...
QuGate provides these patterns as a **multi-tenant contract** — anyone can create and configure gates through transactions, no new contract deployments needed. One codebase to review instead of many custom implementations. Gates are composable: the output of one gate can feed into another, enabling complex payment pipelines from simple building blocks.
...
Full byte-level layouts are documented in the wire format tables above.
...
- **Dust protection**: Sends below the minimum amount (default 10 QU) are burned, preventing spam
...
| Active Gates | Multiplier | Cost (at 1,000 QU base) |
|---|---|---|
| 0–1,023 | 1× | 1,000 QU |
| 1,024–2,047 | 2× | 2,000 QU |
| 2,048–3,071 | 3× | 3,000 QU |
| 3,072–4,095 | 4× | 4,000 QU |
...
QuGate's most powerful feature is **composability**. Gates can be chained together to create complex payment flows that would otherwise require custom smart contracts.
...
**Result**: 40,000 QU entered the pipeline. Gate A split correctly (28k/12k). Wallet B chained 28k into Gate B across two rounds. Gate B accumulated, triggered at threshold, and forwarded all to Wallet C. Total to Wallet C: 40,000 QU. ✅
...
| Metric | Result |
|--------|--------|
| Gates created | 29 across all 5 modes |
| Transactions sent | 29/29 (100% success) |
| Gates verified receiving funds | 10/10 sampled |
| Gates closed | 29/29 (100% success) |
| Slot reuse | 5/5 from free-list ✅ |
| Node stability | ✅ STABLE (4+ hours) |
| Memory impact | Zero growth from gate operations |

**Note**: 21 of 50 create transactions were dropped due to CLI nonce collisions when sending 5 transactions rapidly from the same address. This is a testnet CLI limitation, not a contract bug. Each successfully created gate worked perfectly.
...
6. **THRESHOLD refund goes to owner, not senders**: If a THRESHOLD gate is closed or expires before reaching its target, the held balance refunds to the gate owner — not the individual contributors. This is a deliberate simplification (tracking per-sender contributions would require unbounded state).
...
| `contract_qugate.cpp` | Unit test suite (40 tests, Google Test) |
| `README.md` | Definitive technical reference |
...
- **DAO treasury** — Chain gates to encode treasury policy: revenue → SPLIT (ops, grants, reserves) → grants portion feeds into THRESHOLD that releases when enough accumulates.
...
- **Trustless escrow** — THRESHOLD + CONDITIONAL chain: only buyer can deposit, funds release when agreed amount is reached.
- **Payment batching** — THRESHOLD gate accumulates small payments and forwards in efficient batches.
- **Revenue pipelines** — Chain SPLIT gates for multi-layer distribution (merchant → platform → operations → marketing).
...
| **Revenue pipeline** | SPLIT → SPLIT (chained) | 80/20 then 50/50 on the 80% | Multi-layer distribution without intermediaries |
| **Funding with escrow** | CONDITIONAL → THRESHOLD (chained) | Whitelist senders, accumulate to target | Only approved backers, funds release at milestone |
...
- **Minimum send amount**: 10 QU (default) — prevents dust spam, burned on rejection
...
Built by **fyllepo** — developed and tested on Ubuntu 24.04 using Qubic Core-Lite for local testnet validation. All testing conducted with real contract execution on a live (local) Qubic node, not simulations.

Also maintaining [qubic-mcp](https://github.com/fyllepo/qubic-mcp) — an MCP server for querying Qubic smart contracts. Contributions welcome.
